...
p1 <- "~/Documents/Columbia/Fall 2016/Applied Data Anaylsis/Project4/Project4_data/data"
p2 <- dir(p1,recursive = TRUE)
name <- read.table('common_id.txt')

###extract sound,meta and musicbrainz info
...
for (i in 1:length(p2)) {
  ...
  meta[[i]] <- h5read(paste0(p1,'/',p2[i]),"/metadata")
  musicbrainz[[i]] <- h5read(paste0(p1,'/',p2[i]),"/musicbrainz")
}

###extract length of deeper info inside sound
...
###use data.fram to catch all the information

sound.all <- as.data.frame(cbind(sound.bars.n,sound.beats.n,sound.sections.n,sound.segments.n,sound.tatums.n))
save(sound.all,file = paste0("sound.RData"))
save(sound.bars.n,file = paste0("sound_bars.RData"))
save(sound.beats.n,file = paste0("sound_beats.RData"))
save(sound.sections.n,file = paste0("sound_sections.RData"))
save(sound.segments.n,file = paste0("sound_segments.RData"))
save(sound.tatums.n,file = paste0("sound_tatums.RData"))
